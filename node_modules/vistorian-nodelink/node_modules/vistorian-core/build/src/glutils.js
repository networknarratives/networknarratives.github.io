...
//module glutils {
export function makeAlphaBuffer(array, stretch) {
    const buffer = new Float32Array(array.length * stretch); // three components per vertex
...
export function addBufferedHatchedRect(vertexArray, x, y, z, width, height, colorArray, c) {
    const HATCH_NUM = 3;
    const LINE_WIDTH = 1;
...
export function addBufferedRect(vertexArray, x, y, z, width, height, colorArray, c) {
    width = width / 2;
    height = height / 2;
...
export function addBufferedCirlce(vertexArray, x, y, z, radius, colorArray, c) {
    const segments = 11;
    const angle = Math.PI / (segments / 2);
    for (let i = 0; i < segments; i++) {
        vertexArray.push([x + Math.cos(i * angle) * radius, y + Math.sin(i * angle) * radius, z], [
...
export function addBufferedDiamond(vertexArray, x, y, z, width, height, colorArray, c) {
    width = width / 2;
    height = height / 2;
...
export function createDiagonalCross(w, h, color, lineThickness) {
    w = w / 2;
    h = h / 2;
    const geom = new THREE.Geometry();
    geom.vertices = [
...
export function makeBuffer4f(array) {
    const buffer = new Float32Array(array.length * 4); // three components per vertex
    for (let i = 0; i < array.length; i++) {
...
export function updateBuffer(buffer, array, size) {
    for (let i = 0; i < array.length; i++) {
...
export function createText(string, x, y, z, size, color, weight = "normal", align = "left") {
    const textGeom = new THREE.TextGeometry(string, {
    ...
    const label = new THREE.Mesh(textGeom, textMaterial);
    if (align == "right") {
...
export class WebGL {
    ...
    render() {
        ...
        for (let i = 0; i < this.elementQueries.length; i++) {
            if (this.elementQueries[i].updateAttributes ||
...
let webgl;
export function initWebGL(parentId, width, height, params) {
...
export function setWebGL(scene, camera, renderer, canvas) {
    webgl = new WebGL();
    webgl.camera = camera;
    webgl.scene = scene;
...
export class WebGLElementQuery {
    ...
    filter(f) {
        ...
        const q = new WebGLElementQuery().data(arr);
...
/// METHODS ///
///////////////
export function setStyle(element, attr, v, query) {
    switch (attr) {
        case "fill":
            if (query.shape == "text")
...
// var textCtx
export function setText(mesh, text, parConfig) {
...
//     // return mesh;
// }
export function setX1(mesh, v) {
    mesh.geometry.vertices[0].x = v;
}
export function setY1(mesh, v) {
    mesh.geometry.vertices[0].y = v;
}
export function setX2(mesh, v) {
    mesh.geometry.vertices[1].x = v;
}
export function setY2(mesh, v) {
...
//     }
// }
export function createG(dataElements, scene) {
...
export function createCirclesNoShader(dataElements, scene) {
    let material;
    let geometry;
    const visualElements = [];
    let c;
...
export function createCirclesWithBuffers(query, scene) {
    const dataElements = query.dataElements;
...
export function createRectangles(dataElements, scene) {
    let material;
    let geometry;
    const visualElements = [];
    let c;
    for (let i = 0; i < dataElements.length; i++) {
        ...
        const wireframe = new THREE.Line(geometry, new THREE.LineBasicMaterial({
            color: 0x000000,
            transparent: true,
            linewidth: 1,
        }));
...
export function createPaths(dataElements, scene) {
    let material;
    let geometry;
    const visualElements = [];
    let c, p;
...
export function createPolygons(dataElements, scene) {
    let material;
    let geometry;
    const visualElements = [];
    let c, p;
...
export function createLines(dataElements, scene) {
    let material;
    let geometry;
    const visualElements = [];
    let c, p;
...
export function createWebGLText(dataElements, scene) {
    const visualElements = [];
    let mesh;
    for (let i = 0; i < dataElements.length; i++) {
...
export function createPath(mesh, points) {
...
export function createPolygon(mesh, points) {
    const vectors = [];
...
export class WebGLInteractor {
    ...
    intersectCircles(selection) {
        ...
        for (let i = 0; i < selection.dataElements.length; i++) {
            d = Math.sqrt(Math.pow(this.mouse[0] - selection.x[i], 2) +
    ...
    intersectPaths(selection) {
        ...
        for (let i = 0; i < selection.visualElements.length; i++) {
            ...
            for (let j = 1; j < e.geometry.vertices.length; j++) {
                ...
                if (distToSegmentSquared({ x: this.mouse[0], y: this.mouse[1] }, v1, v2) <
                    3) {
                    intersectedElements.push(selection.dataElements[i]);
                    found = true;
                    break;
...
export class CheckBox {
    constructor() {
        this.selected = false;
        this.changeCallBack = () => null; // BEFORE Function;
...
export class THREEx {
     ...
     */
    drawImage(...args /* same params as context2d.drawImage */) {
        // call the drawImage
