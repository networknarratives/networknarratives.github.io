import { isValidIndex } from "./dynamicgraphutils";
...
export class DataManager {
    ...
    importData(session, data) {
        ...
        if (this.isSchemaWellDefined(data)) {
            ...
            const graphForCaching = new DynamicGraph();
            ...
            if (doubleCheckSave) {
                const testGraph = new DynamicGraph();
    ...
    saveToStorage(dataName, valueName, value, replacer) {
        ...
        while (typeof compress === "undefined") {
            true;
        }
        let stringToSave;
        if (stringifyResult.length > 1024 * 1024 * 4)
            stringToSave = compress(stringifyResult);
        else
            stringToSave = stringifyResult;
        localStorage[this.sessionDataPrefix +
            this.SEP +
            this.session +
            this.SEP +
            dataName +
            this.SEP +
            valueName] = stringToSave;
    }
    ...
    getFromStorage(dataName, valueName, reviver, state) {
        ...
        const storedResult = localStorage[this.sessionDataPrefix +
            this.SEP +
            this.session +
            this.SEP +
            dataName +
        ...
        if (storedResult && storedResult != "undefined") {
            ...
            console.log({ compress: compress, decompress: decompress });
            ...
            if (storedResult == "true") {
                ...
                parseText = decompress(storedResult);
